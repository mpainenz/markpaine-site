export const site = {
  name: 'Mark Paine',
  domain: 'markpaine.dev',
  baseUrl: 'https://markpaine.dev',
  description:
    'Senior CI/CD and platform engineer with 20 years building and operating high-availability telecom, cloud, and payment systems across AWS, Azure, and GCP.',
  location: 'Auckland, New Zealand',
  mobileLocation: 'Auckland, NZ',
  email: 'mpainenz@gmail.com',
  prompt: 'mark@paine:~$',
  status: 'Building',
  currentRole: 'Senior Developer — CI/CD',
  currentEmployer: 'House of Doge',
  currentFocus: 'Platform engineering / SRE',
  founderSummary: 'Solus Designs Ltd — maker of Tensor Relay, distributed AI inference at scale',
  historySummary: '20y telecom-grade backend @ One New Zealand',
  social: {
    github: 'https://github.com/mpainenz',
    linkedin: 'https://www.linkedin.com/in/markpainenz/',
    instagram: 'https://www.instagram.com/mpainenz',
  },
  repositoryUrl: 'https://github.com/mpainenz/markpaine-site',
  pdfPath: '/Mark-Paine-CV.pdf',
  portraitPath: '/headshot.jpg',
  footerDecoration: 'new-zealand',
  keywords: [
    ['Multi-Cloud', 'DevOps / SRE', 'Kubernetes', 'IaC'],
    ['CI/CD', 'Linux', 'Full-Stack', 'ML / AI Inference'],
  ],
};

/** @type {Record<string, string>} */
export const keywordLinks = {
  'Multi-Cloud': 'https://en.wikipedia.org/wiki/Multicloud',
  'DevOps / SRE': 'https://en.wikipedia.org/wiki/Site_reliability_engineering',
  Kubernetes: 'https://en.wikipedia.org/wiki/Kubernetes',
  IaC: 'https://en.wikipedia.org/wiki/Infrastructure_as_code',
  'CI/CD': 'https://en.wikipedia.org/wiki/CI/CD',
  Linux: 'https://en.wikipedia.org/wiki/Linux',
  'Full-Stack': 'https://en.wikipedia.org/wiki/Solution_stack',
  'ML / AI Inference': 'https://en.wikipedia.org/wiki/Machine_learning',
};
